...
This project was created during my postgraduate degree, starting as a bonus objective but slowly evolved into its own standalone tool. Because of it's origins and deadlines the project is ad hoc rather than general purpose. My goals are to experiment and explore procedural modeling practices using python in Autodesk Maya.
...
Let's start with the windows, this should be easy, just loop over the height and
...
Each component has its own logic, but they share a similar structure.
...
<span style="display: flex; flex-direction: row; gap: 18px; float: right; padding: 15px">
  <div
    style={{
...
<ArticleFigure
    imagePath="maya-building-generator/fig_debug.png"
    side="left"
    width="350"
    alt="Figure showing the debugging process."
/>

Overall, I’m satisfied with the result, given the time I had, although there are several areas I'd look to for improvement.
...
Because of time constraints and limited modeling skills I made only a few variations of the asset kit models.
...
Because of its specialization additionally user control and generalization would further the tool's usefulness.
...
Overall, unless I want to punish myself, I'm not keen on doing more procedural modeling in Autodesk Maya.
